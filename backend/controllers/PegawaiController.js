...
const tambahPegawai = (req, res) => {
  upload.single("image")(req, res, (err) => {
    ...
    const sqlQuery =
      "INSERT INTO pegawai (jabatan, nama, nip, image) VALUES (?,?,?,?)";
...
const deletePegawai = (req, res) => {
  ...
  db.query(sqlQuery, [id], (err, result) => {
    if (err) {
      return res
        .status(500)
        .json({ Status: "Error", Error: "Error deleting pegawai", err });
    }
...
const updatePegawai = (req, res) => {
  const id = req.params.id;
  const sqlQuery =
    "update pegawai set jabatan = ?, nama = ?, nip = ? where id = ?";
  db.query(
    sqlQuery,
    [req.body.jabatan, req.body.nama, req.body.nip, id],
    (err, result) => {
